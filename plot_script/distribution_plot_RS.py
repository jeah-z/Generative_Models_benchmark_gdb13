...
def get_parser():
    ...
    parser.add_argument(
        '--test', type=str, default='train_ringsystem.csv', help='Path to the test set'
    )
    parser.add_argument(
        '--config', '-c', type=str, default='distribution_config_RS.csv',
...
if __name__ == "__main__":
    ...
    for metric_i, metric_name in enumerate(metrics):
        ...
        plt.title('Ring Systems (RS)', fontsize=14)
        plt.legend()
        plt.xlabel("Relative mass")
        plt.ylabel("Density")
        plt.savefig(
            os.path.join(config.img_folder, 'RS_'+metric_name+'.pdf')
        )
        plt.savefig(
            os.path.join(config.img_folder, 'RS_'+metric_name+'.png'),
            dpi=250
        )
        plt.close()
